...
def build_extraction_prompt(query: str, context_chunks: List[Dict[str, Any]]) -> str:
    ...
    prompt = (
        ...
        "Pay special attention to technical parameters such as sensor ranges, frame rates (FPS), "
        "hardware used (e.g., RTX 4090), datasets evaluated on (e.g., nuScenes, KITTI), and performance metrics (e.g., mAP). "
        "Include them in your technical_parameters output where applicable.\n"
        "\n"
        f"User Query:\n{query}\n"
        "\n"
        f"Context:\n{context_str}\n"
